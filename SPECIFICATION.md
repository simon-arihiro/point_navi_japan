...
* Admin 手动创建、编辑、删除 categories 和 tags，作为 AI 自动匹配的候选池
...
**适用范围：** categories、tags、services（slug字段）、articles（slug字段）
...
与 categories 管理页完全对称，遵循相同模式（F.1 + F.2）。
参照 `categories/page.tsx` 实现，API 端点替换为 `/api/tags`，prefix 使用 `tag`。
